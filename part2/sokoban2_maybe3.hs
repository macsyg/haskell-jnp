...
checkMove c = res where
  tile = maze (coordX c) (coordY c)
  res = (if tile == Ground || tile == Storage then True
        else False)
        
makeMove :: Direction -> State -> State
makeMove d s 
  | checkMove (adjacentCoord d (stCoord s)) == True = s { stCoord = (adjacentCoord d (stCoord s)), stDir = d} 
  | otherwise = s
