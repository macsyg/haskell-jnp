...
reachableNeighbours :: Coord -> [Coord]
reachableNeighbours coord  = res where
  tile = maze coord
  res = if tile == Box then
    []
  else
    foldList goodToMove [] (adjacentCoords coord) 
...
isClosed :: Maze -> Bool
isClosed testMaze = isGraphClosed (startCoord testMaze) reachableNeighbours (okVertex testMaze)
...
isSane testMaze = res where
  reachableVertices = snd(allReachableVertices (startCoord testMaze) (reachableNeighbours, []))
