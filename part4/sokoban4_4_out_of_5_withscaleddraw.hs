...
-- stan gry
my_world :: State
my_world = S initialCoord initialDirection (getBoxCoords firstMaze) 1
-- stan gry END
...
--pictureOfMaze :: State -> Picture 
--pictureOfMaze s = pictures [translated (fromIntegral x) (fromIntegral y) (drawTile((addBoxes (boxCoords s) (removeBoxes (getMazeDescription s)) (C x y)) )) | x <- [-10..10], y <- [-10..10]]
...
draw :: State -> Picture
draw s = res where
  res = if isWinning s then winScreen else (atCoord (playerCoord s) (player2 (playerDir s))) & pictureOfMaze s
