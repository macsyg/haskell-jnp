...
etap4 :: IO()
etap4 = drawingOf(pictureOfBools (mapList isBoth (appendList mazes badMazes)))
...
handleEvent (KeyPress key) s
    ...
    | isWinning s = s
    | key == "N" = if levelNumber s < numberOfMazes then s{ playerCoord = startCoord (nth mazes (levelNumber s + 1)), 
                                                               boxCoords = (getBoxCoords (nth mazes (levelNumber s + 1))),
                                                               levelNumber = (levelNumber s + 1),
                                                               numOfMoves = 0}
                   else s
