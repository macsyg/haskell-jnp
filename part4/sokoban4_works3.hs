...
--test = print (allReachableVertices (reachableNeighbours maze) (C 1 (-1)) [])
...
badMaze c
  | abs (coordX c) > 4  || abs (coordY c) > 4  = Blank
  | coordX c == -4 && coordY c == 2 = Blank
  | abs (coordX c) == 4 || abs (coordY c) == 4 = Wall
  | coordX c ==  2 && coordY c <= 0        = Wall
  | coordX c ==  3 && coordY c <= 0        = Storage
  | coordX c >= -2 && coordY c == -3        = Box
  | otherwise                = Ground
...
badMaze2 c
  ...
  | abs (coordX c) == 4 || abs (coordY c) == 4 = Wall
  | coordX c ==  2 && coordY c <= 0        = Wall
  | coordX c ==  3 && coordY c <= 1        = Storage
  | coordX c >= -2 && coordY c == -3        = Box
  | otherwise                = Ground
  
badMaze3 :: Coord -> Tile
badMaze3 coord = res where
  yVal = (listLength badMaze3List + 1) `div` 2 + (coordX coord)
  xVal = (listLength (nth badMaze3List 1) + 1) `div` 2 + (coordX coord)
  res = nth (nth badMaze3List yVal) xVal
...
badMazes = [firstBadMaze, secondBadMaze]
numberOfMazes = 2
-- poziomy END
