...
-- stan gry
my_world :: State
my_world = S initialCoord initialDirection (getBoxCoords (nth mazes 1)) 1 0
-- stan gry END
...
winScreen state = scaled 1 1 (lettering ("Poziom ukończony, liczba ruchów:" <> (T.pack (show (numOfMoves state)))) ) -- jak dodać liczbę ruchów do tekstu???
...
firstMaze = Maze initialCoord maze
secondMaze = Maze initialCoord maze2
mazes = [firstMaze, secondMaze]
numberOfMazes = 2
...
pictureOfMaze state = res where 
  ...
  reachableVertices = snd(allReachableVertices coord ((reachableNeighbours (getMazeDescription state)), []))
...
makeMove :: Direction -> State -> State -- updated
...
adjacentCoord :: Direction -> Coord -> Coord -- updated
...
handleEvent :: Event -> State -> State -- updated
handleEvent (KeyPress key) s
    | key == " " = if isWinning s then 
                      if levelNumber s < numberOfMazes then s{ playerCoord = startCoord (nth mazes (levelNumber s + 1)), 
                                                               boxCoords = (getBoxCoords (nth mazes (levelNumber s + 1))),
                                                               levelNumber = (levelNumber s + 1),
                                                               numOfMoves = 0}
                      else s{ playerCoord = startCoord (nth mazes 1), 
                              boxCoords = (getBoxCoords (nth mazes 1)),
                              levelNumber = 1,
                              numOfMoves = 0}
                   else s
    | isWinning s = s
...
isGraphClosed :: Eq a => a -> (a -> [a]) -> (a -> Bool) -> Bool -- updated
...
allReachableVertices :: Eq a => a -> ((a -> [a]), [a]) -> ((a -> [a]), [a]) -- updated
...
reachableNeighbours :: (Coord -> Tile) -> Coord -> [Coord] -- updated
