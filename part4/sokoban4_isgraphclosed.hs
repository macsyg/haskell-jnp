...
isGraphClosed initial neighbours isOk = allList isOk (third (goodVertex initial (neighbours, isOk, [])))

goodVertex :: Eq a => a -> ((a -> [a]), (a -> Bool), [a]) -> ((a -> [a]), (a -> Bool), [a])
goodVertex vertex (neighbours, isOk, acc)
  | isOk vertex && not(elemList vertex acc) = foldList goodVertex (neighbours, isOk, vertex:acc) (neighbours vertex)
...
getNeighbours coord = mapList (\dir -> adjacentCoord dir coord) (dirList)

isOk :: Coord -> Bool
isOk coord 
  | elemList (maze coord) [Ground, Storage, Box] = allList (\adjCoord -> not(maze adjCoord == Blank))(getNeighbours coord)
  | otherwise = False
